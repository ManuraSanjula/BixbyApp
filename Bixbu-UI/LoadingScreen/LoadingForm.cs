...
public class LoadingForm : Form
{
    ...
    private Bitmap BlurImage(Image image, int blurAmount)
    {
        ...
        using (var g = Graphics.FromImage(blurredImage))
        {
            ...
            g.DrawImage(image, new Rectangle(0, 0, blurredImage.Width, blurredImage.Height), 0, 0, image.Width,
...
public class GaussianBlurFilter : IDisposable
{
    ...
    public void Apply(Bitmap image)
    {
        ...
        var bitmapData = image.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadWrite,
    ...
    private void CalculateKernel()
    {
        ...
        for (var i = 0; i < size; i++) kernel[i] /= total;
    }
}