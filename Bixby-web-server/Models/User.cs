...
public class User : IEquatable<User>
{
    ...
    public bool Equals(User? other)
    {
        ...
        return Id.Equals(other.Id) && FirstName == other.FirstName && LastName == other.LastName &&
               Email == other.Email && Address == other.Address && Password == other.Password && Pic == other.Pic &&
               EmailVerify == other.EmailVerify && Tokens.Equals(other.Tokens) &&
               UserAuthTokens.Equals(other.UserAuthTokens);
    }
    ...
    {
        var verficationCode = new VerficationCode();
    ...
    public bool IsTokenExpired(string? token)
    {
        ...
        {
            var isExpired = false;
            var verficationCode = Tokens[token];
            if (verficationCode != null)
            {
                if (verficationCode.valid)
                {
                    var storedDateTime = DateTime.Parse(verficationCode.dateTimeKey);
                    var currentDateTime = DateTime.Now;
                    var elapsed = currentDateTime - storedDateTime;
    ...
    private sealed class UserEqualityComparer : IEqualityComparer<User>
    {
        public bool Equals(User x, User y)
        {
            ...
            return x.Id.Equals(y.Id) && x.FirstName == y.FirstName && x.LastName == y.LastName && x.Email == y.Email &&
                   x.Address == y.Address && x.Password == y.Password && x.Pic == y.Pic &&
                   x.EmailVerify == y.EmailVerify && x.Tokens.Equals(y.Tokens) &&
                   x.UserAuthTokens.Equals(y.UserAuthTokens);
        }
